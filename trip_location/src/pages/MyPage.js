...
const MyPage = () => {
  ...
  const travelList = useQuery(['travelList'], async () => {
    ...
    try {
      const response = await axios.get('http://43.201.170.139/api/v1/travel/plan/list',option)
      return response;
    }catch (error) {
      alert('여행 일정이 없습니다.')
      return error;
  ...
  const review = useQuery(['review'], async () => {
    try {
      ...
      const response = await axios.get(`http://43.201.170.139/api/v1/review/mylist`, option)
      return response;
    } catch (error) {
      return error;
    }
