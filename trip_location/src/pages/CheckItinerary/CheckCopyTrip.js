...
const CheckCopyTrip = () => {
    ...
    const principal = useQuery(["principal"], async () => {
        ...
        const response = await axios.get('http://43.201.170.139/api/v1/user/principal', { headers: { Authorization: localStorage.getItem("accessToken") } });
        return response;
      }, {
        enabled: refresh,
        onSuccess : (response) => {
    ...
    const copyReviewInfo = useQuery(['info'], async () => {
        try {
                const response = await axios.get('http://43.201.170.139/api/v1/travel/plan/info/copy', {
                    params: {
    ...
    const updateTravelInfo = useMutation(async (travelPlan) => {
        ...
        const option = {
            ...
            try {
              const response = await axios.put(`http://43.201.170.139/api/v1/travel/plan/update/${travelId}`, travelPlan, option);
    ...
    const requestData = useMutation(async (updatedScheduleData) => {
        ...
        try {
          const response = await axios.post("http://43.201.170.139/api/v1/travel/plan/save", updatedScheduleData, option)
