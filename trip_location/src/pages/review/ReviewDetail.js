...
const ReviewDetail = () => {
  ...
  const travelInfoReview = useQuery(['infoReview'], async () => {
    try {
      const response = await axios.get('http://43.201.170.139/api/v1/travel/plan/info/review', {
        params: {
  ...
  const getReviewDetails = useQuery(['getReviewDetails'], async () => {
    try {
      ...
      const response = await axios.get(`http://43.201.170.139/api/v1/review/list/${searchParams.get('reviewId')}`)
      return response;
    }catch (error) {
