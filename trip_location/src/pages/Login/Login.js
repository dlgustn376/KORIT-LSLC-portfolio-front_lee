...
const Login = () => {
    ...
    const googleAuthLoginClickHandler = () => {
        window.location.href= "http://43.201.170.139/oauth2/authorization/google"
    }

    const naverAuthLoginClickHandler = () => {
        window.location.href= "http://43.201.170.139/oauth2/authorization/naver"
    }

    const kakaoAuthLoginClickHandler = () => {
        window.location.href= "http://43.201.170.139/oauth2/authorization/kakao"
    }
    ...
    const signInUser = useMutation(async (loginData) => {
        try {
            ...
            const response = await axios.post(`http://43.201.170.139/api/v1/auth/login`, loginData, option);
